...
: Laser(max_beams, map)
{
  ...
  std::cout << "Init Tracer" << std::endl;
}
LikelihoodFieldModelProb::~LikelihoodFieldModelProb() 
{
  std::cout << "LikelihoodFieldModelProb Destructor" << std::endl;
...
{
  // RCLCPP_INFO(get_logger(), "sensorFunction");
  printf("sensorFunction\n");
  auto tracer = opentelemetry::trace::Provider::GetTracerProvider()->GetTracer("sensorFunction");
  auto span = tracer->StartSpan("sensorFunction");
  ...
  auto span1 = tracer->StartSpan("compute_sample_weights");
  // span->AddEvent("compute_sample_weights");
  ...
  if (do_beamskip) {
    ...
    auto span2 = tracer->StartSpan("sample_count_for_loop");
    // span->AddEvent("sample_count_for_loop");
    for (j = 0; j < set->sample_count; j++) {
      sample = set->samples + j;
      pose = sample->pose;
